...
class PatchEmbedding(nn.Module):
    ...
    def __init__(
        ...
        """Create the strided projection layer and patch normalization."""
        super().__init__()
        self.patch_size = patch_size
        self.proj = nn.Conv1d(
            in_channels, d_model, kernel_size=patch_size, stride=patch_size, bias=False
        )
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        """Create learnable position embeddings and dropout."""
        super().__init__()
...
class ECGTransformer(nn.Module):
    ...
    def __init__(
        ...
        """Assemble patch embedding, encoder stack, and regression head."""
        super().__init__()
...
def _warmup_cosine_schedule(
    ...
    def lr_lambda(step: int) -> float:
        """Return the learning-rate multiplier for a training step."""
...
def train_transformer(
    ...
    """
    # Local import to avoid circular dependency with the CNN/LSTM module.
    from src.models.deep_learning_ecg import ECGDataset
    ...
    train_ds = ECGDataset(
        data_dir / "train", window_size=window_size, label_col=label_col
    )
