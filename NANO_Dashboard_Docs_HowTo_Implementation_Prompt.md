...
7. **The assistant (Ask the lab)**
   Explain that it is an in-page, operationally grounded NVIDIA assistant, what it is good for (explain the study, unpack HDA, summarize a result, decide when to switch into the operator routes), the starter prompt chips, and the privacy note that prompts are PHI-scrubbed before the same-origin backend sends them to the configured provider.
...
- Compliance banner: `PHI processing zone . HIPAA-compliant audit logging is active. All exports are stripped of identifiers via REDCap proxy; assistant prompts are PHI-scrubbed before provider submission.` plus `IRB Pro00115234` and the session age.
