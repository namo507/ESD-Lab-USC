...
> **Status note (2026-07-13):** This is a historical audit snapshot, not the current workflow contract. The obsolete `sync_local_llm.yml` workflow has been removed; current policy is enforced by `scripts/check_github_workflows.py` and the Ollama assistant runbook.
