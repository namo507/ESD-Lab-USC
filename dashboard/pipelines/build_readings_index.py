...
def _clean_extracted_text(value: str) -> str:
    ...
    cleaned = re.sub(
        r"\b(?:e-?mail address|email address)\s*:\s*\S+",
        " ",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def _strip_chapter_prefix(text: str) -> str:
    return re.sub(
        rf"^CHAPTER\s+{CHAPTER_LABEL_PATTERN}\s+", "", text, flags=re.IGNORECASE
    )
...
def is_generic_title(title: str) -> bool:
    lowered = _normalize_whitespace(title).lower()
    return lowered in GENERIC_TITLES or any(
        token in lowered for token in GENERIC_TITLE_TOKENS
    )
...
def infer_source(*texts: str) -> str:
    ...
    if (
        "advances in child development and behavior" in lowered
        or "advances in child" in lowered
    ):
        return "Advances in Child Development and Behavior"
    if (
        "child development perspectives" in lowered
        or "child dev perspectives" in lowered
    ):
...
def infer_category(*texts: str) -> str:
    ...
    rules = [
        ...
        (
            ...
            ("sound", "words", "language", "communication", "speech", "vocabulary"),
        ),
        ("Math and Spatial", ("math", "spatial", "numeracy")),
        ("Family and Parenting", ("family", "parenting", "community", "engagement")),
        (
            "Physiology and Autonomic",
            ("autonomic", "attachment", "heart", "hrv", "physiology", "vagal"),
        ),
        (
            "Developmental Theory",
            ("ability", "childhood", "concepts", "essentialism", "theory"),
        ),
    ]
...
def extract_authors_from_excerpt(excerpt: str, title: str = "") -> list[str]:
    ...
    for match in re.finditer(
        r"[A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){1,3}", author_block
    ):
...
def build_excerpt(
    text: str, title: str = "", authors: Optional[list[str]] = None
) -> str:
    ...
    if title_prefix and normalized.lower().startswith(title_prefix.lower()):
        has_body_marker = (
            re.search(r"\b(Abstract|ABSTRACT|Introduction|INTRODUCTION)\b", normalized)
            is not None
        )
        has_affiliation_marker = (
            re.search(
                r"\b(University|Department|School|Contents|Corresponding)\b", normalized
            )
            is not None
        )
    ...
    title_match = (
        re.search(re.escape(title), excerpt_source, flags=re.IGNORECASE)
        if title
        else None
    )
    if title_match:
    ...
    excerpt_source = re.sub(
        r"\*+\s*Corresponding authors?.*$", "", excerpt_source, flags=re.IGNORECASE
    )
    ...
    for sentence in re.split(r"(?<=[.!?])\s+", excerpt_source):
        ...
        if cleaned.lower().startswith(
            ("contents ", "keywords ", "series editor", "edited by", "copyright ")
        ):
            continue
...
def extract_pdf_metadata(file_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        title = _clean_pdf_value(
            getattr(metadata, "title", "") or metadata.get("/Title")
        )
        author = _clean_pdf_value(
            getattr(metadata, "author", "") or metadata.get("/Author")
        )
        subject = _clean_pdf_value(
            getattr(metadata, "subject", "") or metadata.get("/Subject")
        )
...
def choose_title(file_path: Path, pdf_title: str, excerpt: str) -> str:
    ...
    if (
        ...
        and not is_noisy_title(cleaned)
        and len(cleaned) >= 6
    ):
        return cleaned
    ...
    if (
        excerpt_title
        and not is_generic_title(excerpt_title)
        and not is_noisy_title(excerpt_title)
    ):
...
def build_reading_record(file_path: Path, base_dir: Path) -> dict:
    ...
    authors = pdf_metadata["authors"] or extract_authors_from_excerpt(
        raw_excerpt, title
    )
    combined_text = " ".join(
        filter(None, [title, subject, raw_excerpt, filename_title, file_path.name])
    )
    ...
    if category == "Grant Materials" and not re.search(
        r"\b(Abstract|ABSTRACT|Introduction|INTRODUCTION)\b", raw_excerpt
    ):
        excerpt = ""
    ...
    return {
        ...
        "year": extract_year(
            build_search_text(title, subject, filename_title, file_path.name)
        ),
        "source": source,
        "category": category,
        "keywords": keywords,
...
def iter_readings(readings_dir: Path) -> Iterable[Path]:
    ...
    files = [
        path
        for path in sorted(readings_dir.rglob("*"))
...
def build_payload(
    readings_dir: Path = DEFAULT_READINGS_DIR, cache_path: Optional[Path] = None
) -> dict:
    ...
    for path in iter_readings(readings_dir):
        ...
        if (
            cached_record
            and cached_record.get("_signature") == signature
            and cached_record.get("_version") == INDEX_VERSION
        ):
    ...
    readings.sort(
        key=lambda item: (item["year"] or 0, item["modified_at"], item["title"]),
        reverse=True,
    )
    ...
    if cache_path is not None:
        _atomic_write_json(
            cache_path, {"version": INDEX_VERSION, "records": next_cache_records}
        )
    ...
    return {
        "meta": {
            ...
            "source_dir": (
                str(readings_dir.relative_to(PROJECT_ROOT))
                if readings_dir.is_relative_to(PROJECT_ROOT)
                else str(readings_dir)
            ),
...
def _existing_index_used_pdf(output_path: Path) -> bool:
    """True when the index already on disk was built with PDF metadata.

    Used to stop a pypdf-less run from replacing a richer index. An unreadable
    or absent file is not a reason to block the write.
    """
    try:
    ...
    return bool(isinstance(meta, dict) and meta.get("pdf_metadata_enabled"))


def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build esd-lab-readings metadata for the dashboard."
    )
    parser.add_argument(
        "--readings-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fail instead of writing a degraded index when pypdf is unavailable.",
    )
    parser.add_argument(
        "--allow-degraded-overwrite",
        action="store_true",
        help=(
            "Permit a pypdf-less run to replace an index that was built with "
            "PDF metadata. Off by default; see the guard in main()."
        ),
    )
    args = parser.parse_args(argv)

    # Without pypdf every PDF yields page_count None, no embedded title, and no
    # excerpt, so category and title inference fall back to the filename. The
    # result is structurally valid and silently much worse -- which is exactly
    # how it used to overwrite a good index without anyone noticing.
    degraded = PdfReader is None
    if degraded:
        print(
            "[build_readings_index] WARNING: pypdf is not installed. Page counts, "
            "embedded titles, and excerpts will be missing, and categories will be "
            "inferred from filenames alone. Install requirements.txt for a full index.",
            file=sys.stderr,
        )
        if args.require_pdf_metadata:
            print(
                "[build_readings_index] refusing to build without pypdf "
                "(--require-pdf-metadata)",
                file=sys.stderr,
            )
            return 1
        if not args.allow_degraded_overwrite and _existing_index_used_pdf(args.output):
            print(
                f"[build_readings_index] refusing to overwrite {args.output}: the "
                "existing index was built with PDF metadata and this run cannot "
                "reproduce it. Install pypdf, or pass --allow-degraded-overwrite.",
                file=sys.stderr,
            )
            return 1
    ...
    if degraded:
        print("  pdf_metadata: DEGRADED (pypdf unavailable)")
    return 0
