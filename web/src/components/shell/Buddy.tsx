...
export const INSIGHTS: Record<string, InsightData> = {
  ...
  "assistant-model-clinical": { term: "Local model policy", body: "ESD Buddy now uses the shared local GGUF catalog. BioMistral 7B Q4 is the preferred clinical tier, while Qwen and SmolLM fallbacks keep the dashboard usable on smaller Docker, laptop, or Kubernetes hosts." },
  "assistant-model-fallback": { term: "Model fallback", body: "Runtime selection checks installed model files and available memory before loading. If the clinical model is absent or too large for the host, the assistant chooses the best present fallback instead of breaking the live dashboard." },
  ...
  "pm-status": { term: "Assistant status", body: "Generation depends on the local GGUF model being ready. BioMistral is the preferred clinical tier when installed with enough memory, and smaller fallbacks keep the Generate action predictable on constrained hosts." },
