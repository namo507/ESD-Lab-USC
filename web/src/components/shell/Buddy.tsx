...
export const INSIGHTS: Record<string, InsightData> = {
  ...
  "redcap-visit-health": { term: "Visit Health Monitor", body: "The Visit Health Monitor cross-checks CSBS completion status across 6m, 9m, and 12m timepoints against REDCap survey queue behavior to catch carry-forward misalignment before it corrupts longitudinal data." },
  "redcap-anomaly-banner": { term: "Carry-forward alert", body: "This banner flags participants whose earlier timepoint CSBS surveys remain incomplete while a later timepoint's visit has already started - the exact failure mode that causes survey data to land in the wrong event column." },
  "redcap-visit-grid": { term: "Visit status grid", body: "Each row shows one participant's CSBS completion status across all three visit timepoints. Anomaly-flagged records sort to the top with a garnet badge. Click any row for the timeline detail drawer." },
  ...
  "redcap-visit-entry": { term: "Visit date entry", body: "Entering a visit date here triggers REDCap's Form Display Logic to disable the prior timepoint's CSBS survey. This is the key coordination step that prevents carry-forward." },
  ...
  "redcap-visit-drawer": { term: "Visit detail", body: "The detail drawer shows one participant's three-timepoint timeline with survey timestamps, risk explanations, and a deep link into the secure REDCap record." },
