...
export const api = {
  get<T>(path: string, schema: ZodType<T, ZodTypeDef, unknown>): Promise<T> {
    return request(path, schema);
  },
  post<T>(path: string, body: unknown, schema: ZodType<T, ZodTypeDef, unknown>): Promise<T> {
    return request(path, schema, { method: "POST", body: JSON.stringify(body) });
  },
  patch<T>(path: string, body: unknown, schema: ZodType<T, ZodTypeDef, unknown>): Promise<T> {
